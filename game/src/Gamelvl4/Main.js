...
function drawStart(){
  ...
  if(consumeKey('Enter')||consumeKey('Space')){
    // Only start if username is set (overlay was dismissed)
    if(currentUsername.length>=2){
      document.getElementById('save-overlay').style.display='none';
      initAudio();spawnBG();startPhase('identity');
    }
  }
}
...
function drawVictory(){
  ...
  ctx.restore();
  if(consumeKey('Enter')){resetGame();phase='start';hideSaveOverlay();showUsernameOverlay();}
}
let _victorySaved=false;
function onVictory(){
  if(!_victorySaved){_victorySaved=true;saveGameProgress(score,true);fetchLeaderboard();}
}
...
function onGameOver(){
  if(!_gameoverSaved){_gameoverSaved=true;saveGameProgress(score,false);}
}

function resetGame(){P.hp=100;score=0;particles=[];fx.toasts=[];fx.msgQueue=[];gameTime=0;walls=[];npcs=[];fragments=[];projectiles=[];phaseData={};helpOpen=false;tutorialOpen=false;tutorialData=null;spawnBG();_victorySaved=false;_gameoverSaved=false;}
...
function draw(dt){
  ...
  if(phase==='victory'){drawVictory();onVictory();Object.keys(kDown).forEach(k=>kDown[k]=false);return;}
  if(phase==='gameover'){drawGameOver();onGameOver();Object.keys(kDown).forEach(k=>kDown[k]=false);return;}
...
/* ══════ BACKEND INTEGRATION ══════ */
function showUsernameOverlay(){document.getElementById('username-overlay').style.display='flex';}
function hideUsernameOverlay(){document.getElementById('username-overlay').style.display='none';}
function showSaveOverlay(){document.getElementById('save-overlay').style.display='block';}
function hideSaveOverlay(){document.getElementById('save-overlay').style.display='none';document.getElementById('leaderboard-box').style.display='none';}
...
document.getElementById('start-btn').addEventListener('click',function(){
  ...
  if(name.length<2){err.style.display='block';input.style.borderColor='#ff4466';input.focus();return;}
  err.style.display='none';
  currentUsername=name;
  hideUsernameOverlay();
  // The canvas start screen will now accept Enter/Space since currentUsername is set
});
...
async function saveGameProgress(finalScore,completed){
  showSaveOverlay();
  const statusEl=document.getElementById('save-status');
  statusEl.textContent='\u23f3 Saving progress...';
  statusEl.style.color='#666';
  try{
    const res=await fetch(`${GAME_API}/save`,{
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        username:currentUsername,level:4,score:finalScore,maxScore:2000,
        levelCompleted:completed,healthRemaining:P.hp,
        phasesCompleted:completed?['identity','sim_swap','password','deepfake','boss']:[phase],
        totalPhases:5,
        stats:{hp:P.hp,maxHp:P.maxHp,phaseReached:phase}
      })
    });
    const data=await res.json();
    if(data.success){statusEl.textContent='\u2705 Progress saved!';statusEl.style.color='#00ff88';}
...
async function fetchLeaderboard(){
  ...
  try{
    const res=await fetch(`${GAME_API}/leaderboard/4`);
    const data=await res.json();
    if(!data.success||!data.leaderboard||!data.leaderboard.length){
      rowsEl.innerHTML='<p style="color:#555;font-size:12px;">No entries yet.</p>';return;}
    rowsEl.innerHTML=data.leaderboard.map((e,i)=>`
      <div style="display:flex;justify-content:space-between;color:#aaa;font-size:12px;padding:3px 0;border-bottom:1px solid #1a2a3a;">
        <span style="color:#bf00ff;margin-right:8px;">#${i+1}</span>
