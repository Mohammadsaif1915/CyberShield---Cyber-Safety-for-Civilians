...
function winGame() {
    ...
    // Save to backend
    saveGameProgress(finalScore, true);
    fetchLeaderboard();
}
...
function failGame(reason) {
    ...
    // Save fail to backend
    saveGameProgress(score, false, 'fail-save-status');
}

function restartGame() { startGame(); }

// ---- BACKEND INTEGRATION ----
async function saveGameProgress(finalScore, completed, statusId) {
    const statusEl = document.getElementById(statusId || 'save-status');
    if (statusEl) {
        statusEl.textContent = '⏳ Saving progress...';
        statusEl.style.color = '#666';
    }
    try {
        const response = await fetch(`${GAME_API}/save`, {
            method: 'POST',
            ...
            body: JSON.stringify({
                ...
                phasesCompleted: Array.from({length: phase + 1}, (_, i) => PHASES[i]),
                totalPhases: 5,
                stats: {
                    baseScore: score,
                    timeBonus: Math.floor(timer * 5),
                    infectionPenalty: Math.floor(infection * 2),
                    infection: Math.round(infection),
                    phasesReached: phase + 1
                }
            })
        });
...
async function fetchLeaderboard() {
    ...
    if (!rowsEl) return;
    try {
        const response = await fetch(`${GAME_API}/leaderboard/2`);
        const data = await response.json();
