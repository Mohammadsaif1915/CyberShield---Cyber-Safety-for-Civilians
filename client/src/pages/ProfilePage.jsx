...
// ✅ FIX: Safe course ID extractor
// p.course can be: populated object, string ID, or null
const getCourseId = (p) => {
  if (!p) return null
  if (p.course && typeof p.course === 'object') return p.course._id || null
  if (p.course && typeof p.course === 'string') return p.course
  if (p.courseId) return typeof p.courseId === 'object' ? p.courseId._id : p.courseId
  return null
}
        ...
        <div className={styles.section}>
          ...
          {loading ? (
            ...
            <div className={styles.progressList}>
              {allProgress.map(p => {
                // ✅ FIXED: safe extraction — never produces /courses/null
                const courseId = getCourseId(p)
                const course   = typeof p.course === 'object' ? p.course : null
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      else toast.error('Course data unavailable')
                    }}
                    ...
                    <div className={styles.progressInfo}>
                      <div className={styles.progressTop}>
                        <span className={styles.progressTitle}>
                          {course?.title || p.courseTitle || 'Course'}
                        </span>
